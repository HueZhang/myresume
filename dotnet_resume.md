...
#### MES制造执行系统（.NET 8.0 / EFCore / MySQL/Redis）

> 面向制造企业的 MES 系统，覆盖生产管理、质量管理、仓储管理、设备管理、排班管理等模块，实现从订单到交付的全流程数字化管控。

- **工艺路线全链路建模** 建模 MES 核心实体关系：产品→工艺路线→BOM→工单→报工，质量管理实现 IQC/IPQC/OQC 三检体系（模板+检测项+检验单），仓储实现出入库主从表+库存余额视图。
- **ERP 集成**：Hangfire定时任务 + 中间表模式对接 SAP，实现生产订单自动同步与报工数据异步回传
- **高并发报工**：RabbitMQ 异步队列 + SqlBulkCopy 批量入库，200+ 工位并发报工，P99 延迟 < 200ms
- **实时看板推送**：SignalR + Redis Backplane，车间大屏与管理端数据秒级同步

#### 智能仓储管理系统（.NET 8.0 / EFCore / MySQL / Hangfire ）

> 基于 .NET 8的多租户 WMS 仓储管理系统，覆盖入库、库内管理、出库全流程，支持多仓库、多货主（3PL）场景，包含完整的 RBAC 权限体系和可配置审批流引擎。

- **完整的出入库状态机**：入库支持 创建→确认→卸货→分拣→上架 五阶段流转，出库支持 创建→确认→拣货→打包→称重→发货→签收 七阶段流转，每个阶段独立跟踪数量、操作人和时间，实现全链路可追溯。
- **库存多维度管理**：库存按 SKU + 库位 + 货主 + 序列号 + 效期 + 批号多维度管理，支持冻结/解冻、移库、盘点调整等库内操作，使用乐观并发控制保障高并发场景下库存数据一致性。
- **可配置审批流引擎**：独立的审批流模块（流程定义→节点→审批人→条件过滤器），可挂载到任意业务单据，支持多级审批和条件路由。
