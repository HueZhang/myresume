## 基本信息

| **姓名**     | 张昊宇                 | **学历**      | 本科 |
| ------------ | ---------------------- | ------------- | -------------- |
| **出生日期** | 1999/11/14             | **手机/微信** | 15290582150    |
| **邮箱**     | zhang_19991114@163.com | **期望薪资**  | 10k            |
| **工作经验** | 5 年                   | **求职方向**  | .NET 后端 / 全栈(偏后端) |

## 个人简介

5 年 .NET 后端开发经验，覆盖 .NET Framework / .NET Core / ASP.NET MVC / Web API。擅长需求分析、系统设计、编码、测试、部署与运维一体化交付；具备并发处理、性能优化、数据库设计与系统解耦能力，熟悉多租户、RBAC 与分布式架构。可独立完成接口设计与技术文档编写，具备 Vue3 中后台开发能力，支持前后端协同交付；了解 MES 系统常见业务与前后端分离架构（Vue + .NET）。

## 技术技能

- **后端开发**：C# / .NET Core / .NET Framework / ASP.NET MVC / ASP.NET Core Web API / LINQ / 异步编程 / 多线程 / DDD / 设计模式
- **通信与集成**：WebSocket / Socket；Web Services（SOAP）/ XML（了解）
- **中间件与消息**：Redis（缓存、限流、Pub/Sub）、RabbitMQ
- **前端技术**：Vue3 / Element Plus / Vue Router / Pinia / Axios / JavaScript / HTML / CSS / Ajax / jQuery
- **数据库**：SQL Server / MySQL / Oracle，复杂查询、视图、存储过程与作业；MongoDB；EF Core / Dapper / SqlSugar
- **系统设计**：RBAC 权限、多租户隔离、接口规范、缓存与性能优化、IOC/DI
- **流程引擎**：Workflow Core（审批流建模、节点配置、层级流转）
- **微服务与 DevOps**：Nacos（注册/配置）、Jenkins 流水线、Docker、Helm、K8s、Nginx
- **桌面开发**：WinForms / WPF + MVVM，CommunityToolkit.Mvvm + WPF-UI
- **工程化与文档**：接口文档、需求/概要/详细设计文档编写
- **运维与排障**：Linux 常用命令、Shell 脚本、日志与性能定位
- **其他**：串口通信（RS232）；AI 工具辅助开发（Codex / Claude / Gemini）

------

## 工作经历

### 河南产权交易中心

*C# 后端开发工程师 | 2025/07 - 至今*

- 负责多个独立系统的需求分析、架构设计与落地，涵盖专家培训考试、电子发票网关、金融保函等业务
- 独立完成接口设计与文档编写，配合前端联调与测试验收，上线后持续运维与优化
- 解决高并发登录、文件转码、第三方 API 集成与成本优化等关键技术问题

### 苏州华墨信息科技有限公司

*C# 后端开发工程师 | 2024/03 - 2025/07*

- 参与 HDIS 血液透析系统的前后端开发、性能优化与模块交付

### 杭州智瑞思科技有限公司

*C# 后端开发工程师 | 2021/06 - 2024/03*

- 负责 HIS 中药库/药房模块开发，完成设备对接与业务流程稳定性优化

------

## 项目经历

### 核心项目

#### 专家培训与考试系统（多端分布式）

- **技术栈**：.NET 10 / Redis / WebSocket / FFmpeg / 七牛云 OSS / 腾讯云短信 / 国密算法
- **关键贡献**：
  - 设计短信登录防刷限流（Redis 滑动窗口），峰值期开考登录稳定率保持在 99%+（内部监控）
  - 实现七牛云分片直传与断点续传，弱网环境上传成功率提升约 30%
  - 基于 Redis 队列 + Webhook 构建异步转码管道，转码任务平均等待时长下降约 40%
  - 设计动态题库与自定义培训模型，支持规则组卷与学习进度控制
  - 多节点 WebSocket 消息路由（Redis Pub/Sub），保障实时推送一致性
  - 基于 Workflow Core 搭建通用审批流：支持按部门/角色/身份配置权限，提交后按层级自动流转与审批

#### 多租户电子发票网关系统

- **技术栈**：.NET 10 / .NET Framework 4 / MongoDB / 设计模式（工厂）
- **关键贡献**：
  - 通过工厂模式标准化第三方开票渠道，实现服务商“即插即用”
  - MongoDB 缓存第三方接口数据 + TTL 过期策略，第三方 API 调用次数下降约 50%

#### HDIS 血液透析信息系统（SaaS）

- **技术栈**：ASP.NET Core / Admin.NET / SqlSugar / Vue3 / Redis / MQ / Helm/K8s
- **关键贡献**：
  - 定制 RBAC 与多租户隔离，业务模块交付周期缩短约 40%
  - 复杂报表查询索引优化 + Redis 缓存，接口响应从约 1000ms 优化至约 200ms
  - 打通电子签平台，提升临床签署覆盖率与合规流程效率
  - 参与跨网闸部署与 Helm 编排，保障多分院环境一致性

#### HIS 医院信息系统 - 中药房/药房模块（WinForms + WPF 桌面端）

- **技术栈**：WPF / WinForms / MVVM（CommunityToolkit.Mvvm）/ WPF-UI / SQL Server / 串口通信（RS232/RS485）/ TCP Socket
- **项目背景**：为中医院/综合医院提供中药房、西药房日常运营管理的桌面端应用，覆盖药品从入库到发药的全流程，支持与自动发药机、煎药机、电子秤等设备对接
- **关键贡献**：
  - 基于 WPF + MVVM 架构重构原 WinForms 药房模块，采用 CommunityToolkit.Mvvm 实现 ViewModel 层解耦，代码可维护性提升约 50%
  - 实现中药房常用功能：药品入库、出库、退药、盘点、调价、效期预警、处方发药（配药/发药确认）、代煎管理
  - 串口协议对接多品牌自动发药机与煎药机（RS232/RS485），通过 TCP Socket 实现设备状态实时监控与异常告警
  - 设计药品批次与效期跟踪机制，支持先进先出（FIFO）出库策略，减少药品过期损耗
  - 优化处方发药流程：处方从医生站传入后，自动分配摆药任务到对应发药窗口，支持叫号、患者确认、处方追溯
  - 实现与 HIS 其他模块（挂号、收费、住院护士站、医生站）的数据交互，确保处方、药品库存、费用的一致性
  - 通过异步任务与后台线程优化 UI 响应，大数据量查询（如跨月库存报表）操作不阻塞主界面
  - 针对医院内网环境进行性能优化：本地缓存常用字典数据、数据库连接池管理、SQL 查询索引优化

### 其他项目（合并展示）

- **金融保函核心业务系统**：工厂 + 策略模式抽象多资方接入；SM4 + 摘要实现报文防篡改与加密传输
- **API 网关与代理服务**：基于 ASP.NET Core + Refit 统一接口代理与鉴权透传
- **ETL 医疗数据集成平台**：数据清洗与标准化、错误重试与告警、Hangfire 批处理与 Serilog 追踪
- **WPF 提效工具**：翻译辅助与任务流转工具，提升国际化与团队协作效率
- **IOT 医疗设备采集系统**：WinForms + 串口协议采集设备数据；CEF 内嵌前端图表展示

------

## 自我评价

- 具备独立架构设计与落地能力，能快速理解业务并高效交付
- 注重代码质量与系统可维护性，善于性能优化与问题排查
- 跨栈能力强，能承担后端核心研发并协同前端交付
- 学习能力强，持续关注新技术并进行实践

