...
五年 .NET 后端开发经验，精通 C#/.NET Core，擅长医疗行业系统架构设计、系统集成、性能优化和复杂报表处理。具备容器化部署、前后端协作能力，主导多个关键模块开发，能独立承担完整系统模块开发任务。近期重点在 HDIS 系统中独立负责对接电子签名、多语言支持、性能优化等核心功能。
...
独立设计多个系统，对接不同服务提供商、性能优化、满足各种要求。
...
**技术栈：** ASP.NET Core + Vue3 + Redis + WebSocket + FFmpeg + MySQL

**项目描述：**  
实现专家在线考试与视频培训系统，支持题库管理、随机试卷生成及学习进度统计。

**核心实现：**
\- 实现题库分类与随机抽题算法
\- 使用 Redis 防止试卷重复生成
\- 使用 FFmpeg 实现视频转码任务队列
\- 使用 WebSocket 实现考试状态实时同步
\- 实现培训计划与课时完成度统计
\- 支持多节点部署

### 电子发票系统

**技术栈：** ASP.NET Core + MongoDB + MySQL

**项目描述：**  实现企业电子发票开票系统，对接多个第三方开票平台。

**核心实现：**
\- 对接企查查，天眼查等厂商
\- 使用 MongoDB 缓存企业信息，减少第三方 API 调用
\- 设计企业信息缓存过期策略
\- 使用工厂模式封装多开票平台接口（金蝶、百望）
\- 对接旧财务系统（.NET Framework4）通过api项目调用

### 保函业务系统

**技术栈：** ASP.NET Core + HttpClient + AES/RSA

**项目描述：**  实现电子保函业务系统，对接保险公司接口完成保函申请流程。
...
**技术栈：** ASP.NET Core + Vue3 + 多数据源

**项目描述：**  实现通用数据查询工具，支持多数据库动态查询。

**核心实现：**
\- 支持 MySQL / SQLServer / MongoDB 多数据源
\- 动态解析数据库表结构
\- 自动生成 SQL 查询语句
\- 实现可视化查询页面
