...
- **分布式 ID**：借鉴**雪花算法**思想，自研分布式 ID 生成器（时间戳 + 随机数 + 节点标识），用于考试订单号，支持多节点全局唯一生成。
...
- **金融保函核心业务系统**：基于 .NET 8 与工厂/策略模式构建了第三方财险对接网关，主导保函申请、承保请求、出函结果回调、状态同步及异常重试等接口开发，通过抽象业务契约实现核心逻辑与大家财险等厂商 SDK 的彻底解耦并支持零侵入扩展其他机构，同时运用 SM4 加密和摘要算法保障大容量业务报文的机密性与防篡改。
